...
- **18 new tests** — store actions (switchIconTab's explorer case, 4 tree actions) + migration coverage. Total: 2,342.

### Not included in this pass

- Keyboard shortcuts for tab switching (⌘2/⌘3)
- Deleting a project auto-collapsing its Explorer tree node
- A cap on how many Explorer nodes persist to localStorage

### Migration

- Existing `preferredMode` values on pinned vaults/smart icons migrate automatically to `activeTab` on first load (one-time, non-destructive).
