...
export const seedDemoData = async (): Promise<boolean> => {
  ...
  const markdownId = noteMap['Markdown Syntax Demo']
  ...
  if (featuresId && markdownId) {
    await db.noteLinks.add({ source_id: featuresId, target_id: markdownId })
  }
